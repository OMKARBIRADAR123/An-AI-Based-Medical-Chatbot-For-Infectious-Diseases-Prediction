...
class main:
    ...
    def widgets(self):
        self.head = tk.Label(self.master, text='Welcome To Login',background="steelBlue", font=('Times New Roman', 20), pady=20)
        self.head.pack()

        self.logf =tk.Frame(self.master, padx=50, pady=20,background="steelBlue")
        tk.Label(self.logf, text='Username: ',background="steelBlue",font=("Times New Roman", 20), pady=5, padx=5).grid(sticky=tk.W)
        tk.Entry(self.logf, textvariable=self.username, bd=5,background="white", font=('', 15)).grid(row=0, column=1)
        tk.Label(self.logf, text='Password: ',background="steelBlue",font=("Times New Roman", 20), pady=5, padx=5).grid(sticky=tk.W)
        tk.Entry(self.logf, textvariable=self.password, bd=5,background="white", font=('', 15), show='*').grid(row=1, column=1)
        tk.Button(self.logf, text=' Login ', bd=3,font=("Times New Roman", 20),background="black",foreground="white", padx=5, pady=5, command=self.login).grid()
